...
impl DatabaseManager {
    ...
    pub async fn new(configs: Vec<DatabaseConfig>) -> Result<Self> {
        if configs.is_empty() {
            return Err(DatabaseError::ConfigError(
                "At least one database configuration is required".to_string(),
            ));
        }
        ...
        for config in configs {
            ...
            let db = Database::connect(opt)
                .await
                .map_err(|e| DatabaseError::ConnectionError(format!("{}: {}", config.name, e)))?;
    ...
    pub fn default(&self) -> Result<&DatabaseConnection> {
        self.connections.values().next().ok_or_else(|| {
            DatabaseError::ConfigError("No database connections available".to_string())
        })
    }
    ...
    pub async fn close(&mut self, name: &str) -> Result<()> {
        if let Some(conn) = self.connections.remove(name) {
            ...
            Err(DatabaseError::DatabaseNotFound(name.to_string()))
        }
    }
}
